...
public class AccountDAO implements com.hh.HHBank.interfaces.ATM.AccountDAO {
	...
	public List<Account> getAllAccts() {
		...
		try {
			accts = (List<Account>) em.createQuery("SELECT a FROM Account a").getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return accts;
	...
	public void updateAccount(Account acct) {
		Account tempAcct = em.find(Account.class, acct.getId());
		tempAcct.setCurrency(acct.getCurrency());
		tempAcct.setStatus(acct.getStatus());
		em.merge(tempAcct);
	}
	...
	public List<Account> getByUserId(long userId) {
		List<Account> userAccts = new ArrayList<Account>();
		try {
			userAccts = (List<Account>) em.createQuery("SELECT a FROM Account a WHERE userid = :userid")
